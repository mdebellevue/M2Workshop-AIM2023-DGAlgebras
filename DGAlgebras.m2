...
export {"DGAlgebra", "DGAlgebraMap", "dgAlgebraMap", "freeDGAlgebra", "setDiff", "natural", "cycles",
	...
	"homologyModule","dgAlgebraMultMap","dgAlgebra"
}
...
freeDGAlgebra (Ring,List) := opts -> (R,degList) -> (
   ...
   toReturn := new DGAlgebra from A;
   A.natural.cache#(symbol dgAlgebra) = toReturn; -- This isn't right, A is not a dg algebra yet
   return toReturn
)     
...
doc ///
  ...
  Description
    Example
      ...
      B.diff
    Text
      Warning: Divided powers have not yet been implemented, so the output of acyclicClosure is only correct up to 2 times the characteristic of the residue field.
  SeeAlso
    (acyclicClosure,Ring)
///
